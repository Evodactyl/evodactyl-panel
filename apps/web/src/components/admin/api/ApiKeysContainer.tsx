...
const ApiKeysContainer = () => {
    ...
    const {
        ...
        error,
        mutate,
    } = useSWR<ApplicationApiKey[]>('/api/application/api-keys', getApplicationApiKeys);
        ...
        createApplicationApiKey(values.description, allowedIps, values.permissions)
            .then((result) => {
                ...
                addFlash({
                    key: 'admin:api-keys',
                    type: 'success',
                    message: 'API key created. Copy the secret token below.',
                });
            })
    ...
    const handleDelete = () => {
        ...
        deleteApplicationApiKey(deleteIdentifier)
            .then(() => {
                addFlash({ key: 'admin:api-keys', type: 'success', message: 'API key deleted.' });
                setDeleteIdentifier(null);
                mutate();
            })
            .catch((error) => {
                setDeleteIdentifier(null);
                clearAndAddHttpError({ key: 'admin:api-keys', error });
            });
    };
    ...
    return (
        ...
        >
            <ConfirmationModal
                ...
                onModalDismissed={() => setDeleteIdentifier(null)}
            >
            ...
            {newSecret && (
                <div css={tw`bg-green-900 border border-green-700 rounded p-4 mb-4`}>
                    <p css={tw`text-green-200 text-sm mb-2`}>
                        Your new API key secret token is shown below. This will not be displayed again.
                    </p>
                    <CopyOnClick text={newSecret}>
                        <pre css={tw`bg-neutral-900 p-3 rounded text-sm text-neutral-200 cursor-pointer break-all`}>
                            {newSecret}
                        </pre>
                    </CopyOnClick>
                </div>
                            ...
                            <Form>
                                ...
                                <div css={tw`grid grid-cols-1 md:grid-cols-2 gap-4 mb-4`}>
                                    ...
                                    <Field
                                        name={'allowedIps'}
                                        label={'Allowed IPs'}
                                        description={'One per line. Leave blank to allow all.'}
                                    />
                                </div>
                                ...
                                <div css={tw`grid grid-cols-1 md:grid-cols-3 gap-3 mb-4`}>
                                    {API_PERMISSION_KEYS.map(({ key, label }) => (
                                        <div
                                            key={key}
                                            css={tw`flex items-center justify-between bg-neutral-800 rounded px-3 py-2`}
                                        >
                                            ...
                                            <div css={tw`flex gap-1`}>
                                                {[
                                                    ...
                                                    <button
                                                        ...
                                                        type={'button'}
                                                        onClick={() =>
                                                            setFieldValue(`permissions.${key}`, option.value)
                                                        }
                                                        css={[
                ...
                <AdminBox title={'API Credentials'} tools={tools} noPadding>
                    {keys.length > 0 ? (
                                ...
                                {keys.map((apiKey) => (
                                    <AdminTableRow key={apiKey.identifier}>
                                        ...
                                        <AdminTableCell className={'text-center'}>
                                            <button
                                                onClick={() => setDeleteIdentifier(apiKey.identifier)}
                        ...
                        <p css={tw`text-center text-sm text-neutral-400 py-6`}>
                            No application API keys have been created.
                        </p>
                    )}
                </AdminBox>
            )}

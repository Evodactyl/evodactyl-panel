...
import { deleteEgg, type Egg, exportEgg, getEgg, updateEgg, updateEggImport, updateEggScript } from '@/api/admin/nests';
...
const EggEditContainer = () => {
    ...
    const {
        ...
        error,
        mutate,
    } = useSWR<Egg>(`/api/application/nests/${nestId}/eggs/${eggId}`, () => getEgg(Number(nestId), Number(eggId)));
    ...
    const submitConfig = (values: any, { setSubmitting }: FormikHelpers<any>) => {
        ...
        updateEgg(Number(nestId), Number(eggId), {
            ...
            .then(() => {
                addFlash({ key: 'admin:egg', type: 'success', message: 'Egg updated.' });
                mutate();
            })
    ...
    const submitScript = (values: any, { setSubmitting }: FormikHelpers<any>) => {
        ...
        updateEggScript(Number(nestId), Number(eggId), {
            ...
            .then(() => {
                addFlash({ key: 'admin:egg', type: 'success', message: 'Install script updated.' });
                mutate();
            })
            .catch((error) => clearAndAddHttpError({ key: 'admin:egg', error }))
            .finally(() => setSubmitting(false));
    ...
    const handleDelete = () => {
        ...
        deleteEgg(Number(nestId), Number(eggId))
            ...
            .catch((error) => {
                setShowDelete(false);
                clearAndAddHttpError({ key: 'admin:egg', error });
            });
    };

    const handleExport = () => {
        clearFlashes('admin:egg');
        exportEgg(Number(nestId), Number(eggId)).catch((error) => clearAndAddHttpError({ key: 'admin:egg', error }));
    };
                    ...
                    <div css={tw`grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4`}>
                        <AdminBox title={'Egg Information'}>
                            ...
                            <div css={tw`mt-4 pt-4 border-t border-neutral-600 flex gap-2`}>
                                <Button color={'primary'} size={'xsmall'} onClick={handleExport}>
                                    Export
                                </Button>
                        ...
                        <div css={tw`flex items-center justify-between`}>
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`text-xs text-neutral-500 mt-1`}>
                                    If you would like to replace settings for this Egg by uploading a new JSON file,
                                    simply select it here and press &quot;Update Egg&quot;. This will not change any
                            ...
                            >
                                {({ isSubmitting }) => (
                                    <Form>
                                        ...
                                        <div css={tw`mb-4`}>
                                            <label css={tw`text-xs uppercase text-neutral-400 block mb-1`}>
                                                Docker Images
                                            </label>
                                            <p css={tw`text-xs text-neutral-500 mb-1`}>
                                                One per line. Format: label|image or just image.
                                            </p>
                                            <textarea
                                                name={'dockerImages'}
                                    ...
                                    <Form>
                                        ...
                                        <div css={tw`grid grid-cols-1 md:grid-cols-2 gap-4 mb-4`}>
                                            <Field
                                                ...
                                                description={'Docker image for the install script.'}
                                            />
                                            <Field
                                                name={'scriptEntry'}
                                                label={'Script Entry'}
                                                description={'Shell entrypoint for the install script.'}
                                            />
                                        </div>
                                        <div css={tw`mb-4`}>
                                            <label css={tw`text-xs uppercase text-neutral-400 block mb-1`}>

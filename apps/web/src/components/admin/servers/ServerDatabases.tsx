...
const rawDataToDatabase = (data: any): ServerDatabase => ({
    ...
    hostId: data.host_id || data.database_host_id,
...
const ServerDatabases = () => {
    ...
    const {
        ...
        error,
        mutate,
    } = useSWR(`/api/application/servers/${server.id}/databases`, () =>
        http
            .get(`/api/application/servers/${server.id}/databases`)
    ...
    useEffect(() => {
        http.get('/api/application/databases')
            .then(({ data }) => {
                setDbHosts(
                    (data.data || []).map((d: FractalResponseData) => ({
                        id: d.attributes.id,
    ...
    const handleDelete = () => {
        ...
        http.delete(`/api/application/servers/${server.id}/databases/${deleteId}`)
            .then(() => {
                addFlash({ key: 'admin:server:databases', type: 'success', message: 'Database deleted.' });
                setDeleteId(null);
                mutate();
            })
            .catch((error) => {
                setDeleteId(null);
                clearAndAddHttpError({ key: 'admin:server:databases', error });
            });
    };
                                        ...
                                        {databases.map((db: ServerDatabase) => {
                                            ...
                                            return (
                                                <AdminTableRow key={db.id}>
                                                    ...
                                                    <AdminTableCell>
                                                        <code>
                                                            {host ? `${host.host}:${host.port}` : `Host #${db.hostId}`}
                                                        </code>
                                                    </AdminTableCell>
                                ...
                                <Form>
                                    ...
                                    <div css={tw`mb-4`}>
                                        ...
                                        <div css={tw`flex items-center`}>
                                            <span
                                                css={tw`bg-neutral-800 border border-neutral-500 border-r-0 rounded-l px-3 py-2 text-sm text-neutral-400`}
                                            >
                                                s{server.id}_
                                            </span>
                                            <input
                                    ...
                                    <div css={tw`mb-4`}>
                                        <Field
                                            ...
                                            description={
                                                'Where connections should be allowed from. Use % for wildcard.'
                                            }
                                        />
                                    </div>
                                    <div css={tw`mb-4`}>
                                    ...
                                    <p css={tw`text-xs text-neutral-500 mb-4`}>
                                        A username and password for this database will be randomly generated after form
                                        submission.
                                    </p>

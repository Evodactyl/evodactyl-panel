...
import { cleanup, createTestAdmin, createTestUser, get, startServer, stopServer } from '../../helpers/setup.js';
...
describe('Application API: Servers', () => {
    ...
    describe('GET /api/application/servers', () => {
        ...
        test('supports ?filter[name]=... with no matches', async () => {
            const res = await get('/api/application/servers?filter[name]=nonexistent_server_xyz', {
                token: adminToken,
            });
            expect(res.status).toBe(200);

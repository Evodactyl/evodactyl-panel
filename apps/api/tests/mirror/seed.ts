...
const _HASHIDS_SALT = 'mirror_test_salt_2026';
...
function getArgs() {
    ...
    return {
        ...
        phpPort: parseInt(get('php-db-port', '13306'), 10),
        tsHost: get('ts-db-host', '127.0.0.1'),
        tsPort: parseInt(get('ts-db-port', '13307'), 10),
    };
}
...
async function seedData(conn: mysql.Connection): Promise<void> {
    ...
    // --- Users ---
    await conn.query(
        `INSERT INTO users (id, uuid, username, email, name_first, name_last, password, language, root_admin, use_totp, gravatar, created_at, updated_at) VALUES
  ...
  `,
    ...
    await conn.query(
        `INSERT INTO allocations (id, node_id, ip, port, server_id, notes, created_at, updated_at) VALUES ${allocValues.join(',')}`,
        allocParams,
    );
  ...
  `,
    ...
    // --- Eggs ---
    await conn.query(
        `INSERT INTO eggs (id, uuid, nest_id, author, name, description, docker_images, startup, config_files, config_startup, config_logs, config_stop, created_at, updated_at) VALUES
  ...
  `,
    ...
    // --- Server ---
    await conn.query(
        `INSERT INTO servers (id, uuid, uuidShort, node_id, name, description, status, skip_scripts, owner_id, memory, swap, disk, io, cpu, oom_disabled, allocation_id, nest_id, egg_id, startup, image, database_limit, allocation_limit, backup_limit, created_at, updated_at) VALUES
  ...
  `,
    ...
    await conn.query(
        `INSERT INTO api_keys (id, user_id, key_type, identifier, token, memo, r_servers, r_nodes, r_allocations, r_users, r_locations, r_nests, r_eggs, r_database_hosts, r_server_databases, created_at, updated_at) VALUES
  ...
  `,
    ...
    await conn.query(
        `INSERT INTO api_keys (id, user_id, key_type, identifier, token, memo, created_at, updated_at) VALUES

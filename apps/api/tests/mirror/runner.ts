...
const PHP_DB_PORT = parseInt(getArg('php-db-port', '13306'), 10);
const TS_DB_PORT = parseInt(getArg('ts-db-port', '13307'), 10);
...
const SCENARIOS: Scenario[] = [
    ...
    },
    {
        ...
        method: 'PATCH',
        path: '/api/application/locations/1',
...
function deepCompareStructure(a: unknown, b: unknown, path: string = '$'): string[] {
    ...
    if (a === null && b !== null) {
        diffs.push(`${path}: PHP=null, TS=${typeof b}`);
        return diffs;
    }
    if (a !== null && b === null) {
        diffs.push(`${path}: PHP=${typeof a}, TS=null`);
        return diffs;
    }
...
async function compareDbState(
    ...
    if (check.field && check.expected !== undefined) {
        ...
        if (phpVal !== check.expected)
            diffs.push(`DB PHP ${check.table}.${check.field}: expected ${check.expected}, got ${phpVal}`);
        if (tsVal !== check.expected)
            diffs.push(`DB TS ${check.table}.${check.field}: expected ${check.expected}, got ${tsVal}`);
    }
...
async function main() {
    ...
    for (const scenario of SCENARIOS) {
        const [phpRes, tsRes] = await Promise.all([fetchEndpoint(PHP_URL, scenario), fetchEndpoint(TS_URL, scenario)]);

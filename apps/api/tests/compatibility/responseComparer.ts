...
function deepCompareStructure(phpVal: unknown, tsVal: unknown, path: string = '$'): string[] {
    const diffs: string[] = [];
...
async function main() {
    ...
    const clientToken = args
        .find((a) => a.startsWith('--client-token='))
        ?.split('=')
        .slice(1)
        .join('=');
    ...
    if (!token) {
        console.error(
            'Usage: bun run tests/compatibility/responseComparer.ts --token=<app_token> --client-token=<client_token> [--php-url=...] [--ts-url=...]',
        );
        process.exit(1);
    }
    ...
    for (const endpoint of ENDPOINTS) {
        ...
        if (!useToken) {
            console.log(
                `  - ${endpoint.method} ${endpoint.path} — SKIPPED (no ${endpoint.client ? 'client' : 'app'} token)`,
            );
            skipped++;
            continue;
        }
        ...
        if (result.statusMatch && result.structureMatch) {
            console.log(
                `  ${statusIcon} ${result.endpoint} — status: ${result.phpStatus}/${result.tsStatus}, structure: match`,
            );
            passed++;
        } else {
            console.log(
                `  ${statusIcon} ${result.endpoint} — status: ${result.phpStatus}/${result.tsStatus}, structure: ${structIcon}`,
            );

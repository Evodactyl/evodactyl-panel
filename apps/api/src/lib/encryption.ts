...
function validMac(payload: LaravelEncryptedPayload, key: Buffer): boolean {
    ...
    return crypto.timingSafeEqual(Buffer.from(calculated, 'hex'), Buffer.from(payload.mac, 'hex'));
}
...
export function decrypt(encryptedValue: string): string {
    const key = getKey();
    const payload: LaravelEncryptedPayload = JSON.parse(Buffer.from(encryptedValue, 'base64').toString('utf-8'));

...
export async function deleteUserCommand(args: string[]): Promise<number> {
    ...
    try {
        ...
        const search = options.get('user') ?? (await ask(rl, 'Search for a user (email, username, or id)'));
        ...
        if (interactive && !(await confirm(rl, `Really delete user "${victim.email}"?`, false))) {

...
export class ServerTransformer extends BaseTransformer {
    ...
    getAvailableIncludes(): string[] {
        return ['allocations', 'user', 'subusers', 'nest', 'egg', 'variables', 'location', 'node', 'databases'];
    }
    ...
    async includeSubusers(server: any): Promise<SerializedResource> {
        ...
        const subusers =
            server.subusers ??
            (await prisma.subusers.findMany({
                where: { server_id: server.id },
                include: { users: true },
            }));
    ...
    async includeDatabases(server: any): Promise<SerializedResource> {
        ...
        const databases =
            server.databases ??
            (await prisma.databases.findMany({
                where: { server_id: server.id },
            }));

...
export class EggTransformer extends BaseTransformer {
    ...
    transform(model: any): Record<string, unknown> {
        const dockerImages =
            typeof model.docker_images === 'string' ? JSON.parse(model.docker_images) : (model.docker_images ?? {});

        let configFiles: unknown = {};
        try {
            const parsed = typeof model.config_files === 'string' ? JSON.parse(model.config_files) : model.config_files;
        ...
        try {
            const parsedStartup =
                typeof model.config_startup === 'string' ? JSON.parse(model.config_startup) : model.config_startup;
            // PHP json_decode with assoc=true converts {} to empty array []
            if (
                parsedStartup !== null &&
                typeof parsedStartup === 'object' &&
                !Array.isArray(parsedStartup) &&
                Object.keys(parsedStartup).length === 0
            ) {
                configStartup = [];
            } else {
        ...
        try {
            const parsedLogs =
                typeof model.config_logs === 'string' ? JSON.parse(model.config_logs) : model.config_logs;
            // PHP json_decode with assoc=true converts {} to empty array []
            if (
                parsedLogs !== null &&
        ...
        const fileDenylist =
            model.file_denylist == null
                ? null
                : typeof model.file_denylist === 'string'
                  ? JSON.parse(model.file_denylist)
                  : model.file_denylist;

        const dockerImagesObj = typeof dockerImages === 'object' && !Array.isArray(dockerImages) ? dockerImages : {};

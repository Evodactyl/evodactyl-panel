    ...
    async handle(server: any, email: string, permissions: string[]): Promise<any> {
        ...
        if (user) {
            ...
            if (server.owner_id === user.id) {
                throw new DisplayException('Cannot add the owner of a server as a subuser.', 422);
            }
            ...
            if (existingSubuser) {
                throw new DisplayException('A subuser with that email address already exists for this server.', 422);
            }
        } else {

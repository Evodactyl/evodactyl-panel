...
export class BuildModificationService {
    ...
    async handle(server: any, data: Record<string, any>): Promise<any> {
        ...
        if (data.allocation_id && data.allocation_id !== server.allocation_id) {
            ...
            if (!alloc) {
                throw new DisplayException(
                    'The requested default allocation is not currently assigned to this server.',
                );
            }
        }
        ...
        try {
            const node =
                updated.nodes ?? server.nodes ?? (await prisma.nodes.findUnique({ where: { id: server.node_id } }));

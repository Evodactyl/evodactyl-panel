 ...
 */
export async function updateNode(nodeId: number, data: Record<string, any>, resetToken: boolean = false): Promise<any> {
    const updateData: Record<string, any> = { ...data };

    if (resetToken) {
        updateData.daemon_token = encrypt(
            crypto
                .randomBytes(DAEMON_TOKEN_LENGTH / 2)
                .toString('hex')
                .slice(0, DAEMON_TOKEN_LENGTH),
        );

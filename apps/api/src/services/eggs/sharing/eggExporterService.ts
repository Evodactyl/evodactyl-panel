...
export async function exportEgg(eggId: number): Promise<string> {
    ...
    try {
        const parsed =
            typeof egg.file_denylist === 'string' ? JSON.parse(egg.file_denylist) : (egg.file_denylist ?? []);
    ...
    const dockerImages =
        typeof egg.docker_images === 'string' ? JSON.parse(egg.docker_images) : (egg.docker_images ?? {});

    const features = typeof egg.features === 'string' ? JSON.parse(egg.features) : (egg.features ?? null);

...
export async function assignAllocations(
    ...
    if (cidrParts.length === 2) {
        ...
        if (Number.isNaN(bits) || bits < CIDR_MAX_BITS || bits > CIDR_MIN_BITS) {
            throw new DisplayException(`CIDR notation must be between /${CIDR_MAX_BITS} and /${CIDR_MIN_BITS}.`, 400);
        }
    }
        ...
        for (const portEntry of data.allocation_ports) {
            ...
            if (rangeMatch) {
                ...
                if (start <= PORT_FLOOR || end > PORT_CEIL) {
                    throw new DisplayException(`Port numbers must be between ${PORT_FLOOR + 1} and ${PORT_CEIL}.`, 400);
                }

                const count = end - start + 1;
                if (count > PORT_RANGE_LIMIT) {
                    throw new DisplayException(`Port range cannot exceed ${PORT_RANGE_LIMIT} ports.`, 400);
                }

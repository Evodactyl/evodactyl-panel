 ...
 */
export async function sendAddedToServerNotification(data: AddedToServerNotification): Promise<void> {
    // In production, send via email/notification service
    console.log(`[Notification] User ${data.userEmail} has been added as a subuser to server "${data.serverName}".`);
}

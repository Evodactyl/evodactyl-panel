...
export function validateStoreNode(body: Record<string, any>): StoreNodeData {
    ...
    if (
        body.memory_overallocate === undefined ||
        typeof body.memory_overallocate !== 'number' ||
        body.memory_overallocate < -1
    ) {
    ...
    if (
        body.disk_overallocate === undefined ||
        typeof body.disk_overallocate !== 'number' ||
        body.disk_overallocate < -1
    ) {
        errors.push('disk_overallocate is required and must be at least -1.');
    }

    if (
        body.daemon_listen === undefined ||
        typeof body.daemon_listen !== 'number' ||
        body.daemon_listen < 1 ||
        body.daemon_listen > 65535
    ) {
        errors.push('daemon_listen is required and must be between 1 and 65535.');
    }

    if (
        body.daemon_sftp === undefined ||
        typeof body.daemon_sftp !== 'number' ||
        body.daemon_sftp < 1 ||
        body.daemon_sftp > 65535
    ) {
        errors.push('daemon_sftp is required and must be between 1 and 65535.');
    }

    if (
        body.daemon_base !== undefined &&
        typeof body.daemon_base === 'string' &&
        !/^\/[\d\w.\-/]+$/.test(body.daemon_base)
    ) {
